...
test.describe("Login no Saucedemo", () => {
  test("validar login com sucesso", async ({ page }) => {
    ...
    const tituloPaginaProdutos = await page
      .locator(".header_secondary_container > span.title")
      .textContent(); //localiza o título da página de produtos
    await expect(tituloPaginaProdutos).toBe("Products"); //verifica se o título da página de produtos está correto
  });

  test("validar mensagem de erro ao tentar logar com senha inválida", async ({
    page,
  }) => {
    ...
    const mensagemErro = await page.getByText(
      "Epic sadface: Username and password do not match any user in this service"
    ); //localiza a mensagem de erro exibida
